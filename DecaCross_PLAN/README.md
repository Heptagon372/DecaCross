...
| 파일 | 내용 |
|---|---|
| [`docs/00_스택결정_Kotlin.md`](docs/00_스택결정_Kotlin.md) | **왜 Kotlin인가** — Rust/Tauri 대비 득실, JVM이 여는 기능 5개, 지불할 비용 2개 |
| [`docs/01_기획서.md`](docs/01_기획서.md) | 무엇을·왜 — 시장, 경쟁 분석, 기능 45개, 로드맵, 수익 모델, 리스크 |
| [`docs/02_설계서.md`](docs/02_설계서.md) | 어떻게 — 아키텍처, PubGrub 매핑, 설치 파이프라인, 보안, DB 스키마 |
| [`docs/03_구현명세_v2.md`](docs/03_구현명세_v2.md) | **정확히 무엇을 타이핑할 것인가** — 타입 정의, 시그니처, 마이그레이션, 완료 판정 |
| [`CLAUDE.md`](CLAUDE.md) | Claude Code 프로젝트 규칙 (불변식·함정·검증 명령) |
| [`prompts/`](prompts/) | Claude Code 실행용 단계별 프롬프트 (00 → 08) |

## 시작하기

```bash
# 1. 이 폴더에서 git 초기화
git init && git add -A && git commit -m "docs: 기획서 v1.1 / 설계서 v1.0 / 구현명세 v2.1-KT"
